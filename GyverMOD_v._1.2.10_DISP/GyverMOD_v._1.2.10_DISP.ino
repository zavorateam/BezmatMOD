...
#define welcome 1              // приветствие (слова ZAVR VAPE при включении), 1 - включить, 0 - выключить
...
#define battery_low 2.5        // нижний порог срабатывания защиты от переразрядки аккумулятора, в Вольтах!
//----------------------------------------НАСТРОЙКИ----------------------------------------

#include <EEPROMex.h>  // библиотека для работы со внутренней памятью платы
#include <LowPower.h>  // библиотека для сна
//#include <GyverPower.h> новая библиотека для сна (в разработке)
...
void setup() {
  Serial.begin(9600);
  //  power.autoCalibrate(); автоматическая калибровка сна
  //  power.hardwareDisable(PWR_ADC); отключаем АЦП и компаратор
...
void loop() {
  ...
  if (wake_up_flag) wake_puzzle();  // вызвать функцию 5 нажатий для пробуждения
  ...
  while (1) {
    ...
    if (vape_state && !vape_btt_f) {
      ...
      if (click_count > 4) {  // если 5 нажатий сделаны за 3 секунды
        wake_status = 1;
        Serial.println("Принято. Просыпаюсь");  // флаг "проснуться"
        break;
      }
    }
    if (!vape_state && vape_btt_f) {
    ...
    if (millis() - wake_timer > 3000) {  // если 5 нажатий не сделаны за 3 секунды
      wake_status = 0;
      Serial.println("Отказ. Меньше пяти нажатий. Продолжаю сон");  // флаг "спать"
      break;
    }
  }
  if (wake_status) {
...
//-------------функция ухода в сон---------------
void good_night() {
  disp_send(BYE);  // попращаться
  delay(500);
  disp.clear();
  Timer1.disablePwm(mosfet);  // принудительно отключить ШИМ на койл
  digitalWrite(mosfet, LOW);  // принудительно отключить койл
  delay(50);
  attachInterrupt(0, wake_up, FALLING);  // подключить прерывание для пробуждения
  delay(50);
  digitalWrite(disp_vcc, LOW);                          // принудительно отключить экран
  LowPower.powerDown(SLEEP_FOREVER, ADC_OFF, BOD_OFF);  // сон, АЦП выкл
}
//-------------функция ухода в сон---------------
...
void service_mode() {
  Serial.println(flag);
  Serial.println(my_vcc_const);
  Serial.println(last_vape);
...
// функция вывода слов на дисплей
...
void calibration() {
  ...
  //после записи в EEPROM калибровку в настройках можно выключить. Теперь модом можно пользоваться
  while (1)
